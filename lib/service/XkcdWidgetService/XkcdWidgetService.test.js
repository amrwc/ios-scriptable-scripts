...
describe('XkcdWidgetService', () => {
	...
	it.each([true, false])('should have created comic widget', withImage => {
	...
	it.each([false, true])('should have created offline comic widget without cache', cacheEnabled => {
		...
		expect(fileManager.joinPath).toHaveBeenCalledWith(DOCUMENTS_DIR + '/' + LOCAL_CACHE_DIRNAME, XKCD_CACHE_FILENAME);
	...
	it.each([true, false])('should have created offline comic widget with cache', withImage => {
		const image = withImage ? jest.fn() : null;
		const title = 'Comic Title';
		...
		fileManager.readString.mockImplementationOnce(filePath => `
		{
			"title": "${title}",
			"image": {
