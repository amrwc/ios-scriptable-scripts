...
test('Should have created comic widget', () => {
	...
	expect(ListWidget.addStack).toHaveBeenCalledTimes(2);
	expect(widgetStack.addSpacer).toHaveBeenCalledTimes(4);
	expect(widgetStack.addText).toHaveBeenCalledTimes(1);
	expect(widgetStack.addText).toHaveBeenCalledWith(title);
	expect(widgetStack.addImage).toHaveBeenCalledTimes(1);
